...
from src.knowledge.species_distribution import SpeciesDistributionService
...
distribution_service = SpeciesDistributionService(expert_system.kb)
...
def answer_question():
    ...
    answered = {}
    for k, v in data.get("answered", {}).items():
        if isinstance(v, str) and v.strip().lower() in {"unknown", "khong_biet", "không_biết"}:
            answered[k] = "unknown"
        else:
            answered[k] = bool(v)
...
# ============================================================
@app.route('/infer_legal_scenario', methods=['POST'])
def infer_legal_scenario():
    data = request.get_json() or {}
    species = data.get("species")
    possession_status = data.get("possession_status", "observe")
    ...
    print(
        f"[LEGAL_SCENARIO] species={species} "
        f"status={possession_status} quantity={quantity}"
    )
    result = expert_system.infer_legal_scenario(species, possession_status, quantity)
    return jsonify(result)


# ============================================================
@app.route('/species_distribution', methods=['GET'])
def species_distribution():
    species = request.args.get("species", "").strip()
    if not species:
        return jsonify({"error": "Missing species"}), 400

    result = distribution_service.get_distribution(species)
    return jsonify(result)
